...
+/::
	...
	while(active){
		IfWinActive, Dota 2
		{
			...
			;Make sure you're not a leader
			PixelGetColor, color, 209, 736
			If (color!=0x36372E && !findMatchAllowed && !isInLobby()){
			...
			If (!isInLobby()){
				...
				If !ErrorLevel{
					...
					;Get lobby slot after click lobby
					if (lobbyClicked && isInLobby()){
						inLobby:=true
					...
					;TODO If you are not accept match and has party and not in lobby, then leave (flag needed)
					;TODO If you are having a siltbreaker dialog
				}
			}
