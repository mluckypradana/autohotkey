...
Loop ;Loop forever
{
    ...
    if (batteryChargePercent > 95)
    {
        ...
        if (acLineStatus == 1)
        {
            ; Only alert if the power lead is connected
            if (batteryChargePercent != 255)
            {
                ; and if the battery is not disconnected
                output := "Unplug, Fn+Q to Battery Saver"
                notifyUser(output)
            }
        }
    }
    ...
    if (batteryChargePercent < 16)
    {
        ; Yes
        if (acLineStatus == 0)
        {
            ; Only alert if the power lead is not connected
            output := "Plug, Fn+Q to Performance"
            notifyUser(output)
        }
    }
